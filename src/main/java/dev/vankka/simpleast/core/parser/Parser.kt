...
//          println("source: $inspectionSource -- depth: ${remainingParses.size}")
  ...
  private fun <R, T: Node<R>, S> logMatch(rule: Rule<R, T, S>, source: CharSequence) {
    if (enableDebugging) {
      println("[$TAG] MATCH with rule pattern: " + rule.matcher.pattern().toString() + " to source: " + source)
    }
  }

  private fun <R, T: Node<R>, S> logMiss(rule: Rule<R, T, S>, source: CharSequence) {
    if (enableDebugging) {
      println("[$TAG] MISS: with rule with pattern: " + rule.matcher.pattern().toString() + " to source: " + source)
    }
  }
